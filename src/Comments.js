...
class Comments extends React.Component {
  ...
  render() {
    ...
    const comments = this.state.expanded
      ? allComments
      : allComments.slice(0, this.numShow());
    ...
    const button = this.state.expanded ? undefined : (
      ...
      >
        {props.comments.length - this.numShow()} More Comment
        {props.comments.length - this.numShow() > 1 ? "s" : ""}
      </button>
    );

    const banner =
      this.props.showBanner === undefined || this.props.showBanner ? (
    ...
    return (
      <div className="comments-container read-more-container">
        {banner}
        <CommentSubmit onSubmit={props.onSubmit} name={props.name} />
